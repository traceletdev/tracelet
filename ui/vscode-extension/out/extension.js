...
let statusBarItem;
let debounceTimer;
class TraceletCodeActionProvider {
    provideCodeActions(document, range, context) {
        ...
        for (const diag of context.diagnostics) {
            ...
            if (ruleId === 'unoptimized-image') {
                ...
                const action = new vscode.CodeAction('Add font-display: swap', vscode.CodeActionKind.QuickFix);
                action.diagnostics = [diag];
                action.edit = new vscode.WorkspaceEdit();
                const line = document.lineAt(range.start.line);
                const lineText = line.text;
                if (lineText.includes('@font-face')) {
                    ...
                    for (let i = range.start.line; i < Math.min(range.start.line + 5, document.lineCount); i++) {
                    ...
                    if (!nextText.includes('font-display')) {
                        ...
                        action.edit.insert(document.uri, new vscode.Position(nextLine, 0), `${indent}font-display: swap;\n`);
                        actions.push(action);
                    }
                }
            }
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tracelet.openConfig', async () => {
...
async function lintDocument(doc) {
    ...
    try {
        ...
        if (errMsg.includes('ENOENT') || errMsg.includes('not found')) {
            vscode.window.showWarningMessage(`tracelet binary not found. Set tracelet.binaryPath in settings.`);
        }
        // Clear diagnostics on error
        diagnosticCollection.set(doc.uri, []);
    }
}
function updateDiagnostics(doc, results) {
    const diagnostics = [];
...
async function probeRoute(url) {
    const binary = getBinaryPath();
    if (!binary) {
        vscode.window.showErrorMessage('tracelet binary not found. Set tracelet.binaryPath in settings.');
        return;
    }
    const config = vscode.workspace.getConfiguration('tracelet');
    const profile = config.get('probe.profile', 'desktop');
    try {
